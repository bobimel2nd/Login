				...
				if (res.body.code === "200") {
					...
					for(var i in filestobeDisplayed){
						filenamesDiv.push(
							<ListItem
								...
								leftCheckbox={<Checkbox value={filestobeDisplayed[i].name} 
									onCheck={(event, value) => self.handleFileCheck(event,value)}/>}
							/>
						)
					}
					...
					fileDivContainer.push(
						<div className="previousfilesContainer">
							...
							<MuiThemeProvider>
								<RaisedButton disabled={self.state.printButtonDisabled} 
									label="Print Files" 
									primary={true} 
									style={style} 
